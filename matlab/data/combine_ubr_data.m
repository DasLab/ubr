...
for q = 2:length( all_d )
    ...
    d_out.r_norm = cat(3, d_out.r_norm, all_d{q}.r_norm);
    d_out.r_norm_err = cat(3, d_out.r_norm_err, all_d{q}.r_norm_err);
    ...
    for n = 1:length( all_d{q}.shape_nomod_idx )
        shape_nomod_idx = all_d{q}.shape_nomod_idx{n};
        for k = 1:length(shape_nomod_idx); shape_nomod_idx(k) = shape_nomod_idx(k) + num_tags; end;
        d_out.shape_nomod_idx{ num_cidx + n } = shape_nomod_idx;
    end
